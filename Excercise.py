...
fig, ax = plt.subplots(2, 4, sharex=True, sharey=True)
fig.suptitle('Ridge lighthouses')
ax[0,0].imshow(imgs[77])
...
In this case, using an ensemble of 10 members. The inputs are split to 10 folds,
and each model is fitted to each fold. A separate validation set is then used to
validate the generalization performance. The final prediction of the ensemble is
the maximum class among separate models' predictions.
